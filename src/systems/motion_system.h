...
public:
    ...
    physx::PxRigidDynamic* createDynamic(const std::vector<physx::PxConvexMesh*>& convexMeshes, glm::vec3 material, glm::vec3 transform, float mass = 10.0f, float sleepT = 0.05f, float linearDamp = 0.0f, float angularDamp = 0.0f, int solverPosition = 4, int solverVelocity = 4, glm::quat rot={0,0,0,0});
    ...
    physx::PxRigidStatic* createStatic(const std::vector<physx::PxTriangleMesh*>& meshes, glm::vec3 material, glm::vec3 transform);
    ...
    void loadConvexObjToPhysX(const std::string& filePath, std::vector<physx::PxConvexMesh*>& convexMeshes);
